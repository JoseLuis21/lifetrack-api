...
func init() {
	cmd, clean, err := dep.InjectChangeCategoryState()
	if err != nil {
		log.Fatalf("failed to start change category state category command: %s", exception.GetDescription(err))
	}
	cleaning = clean

	h := categoryhandler.NewChangeState(cmd, mux.NewRouter().PathPrefix("/live").Subrouter())
