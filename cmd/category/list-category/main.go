...
func init() {
	q, clean, err := dep.InjectListCategoriesQuery()
	if err != nil {
		log.Fatalf("failed to start list category query: %s", exception.GetDescription(err))
	}
	cleaning = clean

	h := categoryhandler.NewList(q, mux.NewRouter().PathPrefix("/live").Subrouter())
