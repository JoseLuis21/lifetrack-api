...
func init() {
	cmd, clean, err := dep.InjectRemoveCategory()
	if err != nil {
		log.Fatalf("failed to start remove category command: %s", exception.GetDescription(err))
	}
	cleaning = clean

	h := categoryhandler.NewRemove(cmd, mux.NewRouter().PathPrefix("/live").Subrouter())
