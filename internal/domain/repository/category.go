...
type Category interface {
	...
	FetchByID(ctx context.Context, id value.CUID) (*aggregate.Category, error)
	// Fetch returns an aggregate.Category slice, accepts multiple filters and params such as nextToken and limit for
	// pagination
	Fetch(ctx context.Context, token string, limit int64, criteria shared.CategoryCriteria) ([]*aggregate.Category,
		string, error)
